...
class DbBackup(models.Model):
    ...
    is_dropbox_token_generated = fields.Boolean(string='Dropbox Token Generated',
    ...
    dropbox_folder = fields.Char('Backup Directory', help='Absolute path for storing the backups',
    ...
    def _compute_is_dropbox_token_generated(self):
        ...
        """
        for rec in self:
            rec.is_dropbox_token_generated = bool(rec.access_token)
    ...
    def set_dropbox_refresh_token(self, auth_code):
        ...
        self.access_token = outh_result.refresh_token

    def _access_dbx(self):
        dbx = None
        try:
            dbx = dropbox.Dropbox(app_key=rec.dropbox_client_id, app_secret=rec.dropbox_client_secret,
                                  oauth2_refresh_token=rec.access_token)
        ...
        if conf_ids:
            for rec in conf_ids:
                ...
                try:
                    ...
                    fp = open(file_path, 'wb')
                    backup_status['local_backup'], backup_status['local_backup_error'] = self._take_dump(rec.name, fp,
                                                                                                         'db.backup',
                                                                                                         rec.backup_type)
                    if rec.upload_dropbox and backup_status['local_backup']:
                        backup_status['dbx_backup'], backup_status['dbx_backup_error'] = self.upload_file(
                            rec.access_token, file_path, f'{rec.dropbox_folder}/{bkp_file}')
                    fp.close()
                ...
                if rec.email_notification:
                    if not backup_status['dbx_backup'] and backup_status['local_backup']:
                        ...
                        self._send_mail_notification(failed='Both Local and DropBox', error=backup_error,
                                                     email=rec.email)
                    else:
                        pass
    ...
    def _remove_from_dropbox(self, days_to_keep, dropbox_folder):
        try:
            dbx = self._access_dbx()
