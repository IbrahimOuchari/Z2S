...
class MsReportStock(models.TransientModel):
    ...
    product_ids = fields.Many2many('product.product', 'rapport_stock_product_rel', 'rapport_stock_id',
                                   'product_id', 'Produit', domain=[('type', '=', 'product')])
    categ_ids = fields.Many2many('product.category', 'rapport_stock_categ_rel', 'rapport_stock_id',
                                 'categ_id', 'Catégories')
    location_ids = fields.Many2many('stock.location', 'rapport_stock_location_rel', 'rapport_stock_id',
    ...
    def print_excel_report(self):
        ...
        datetime_string = self.get_default_date_model().strftime("%d - %m - %Y")
        date_string = self.get_default_date_model().strftime("%d - %m - %Y")
        report_name = 'Rapport de Stock'
        filename = '%s %s' % (report_name, date_string)

        columns = [
            ('No', 5, 'no', 'no'),
            ('Produit', 30, 'char', 'char'),
            ('Catégories', 20, 'char', 'char'),
            ('Emplacement', 15, 'char', 'char'),
            ('Date Réception', 20, 'datetime', 'char'),
            ('Durée Stockage', 15, 'number', 'char'),
            ('Total Stock', 15, 'float', 'float'),
            ('Disponible', 15, 'float', 'float'),
            ('Réservé', 15, 'float', 'float'),
        ]
        ...
        for res in result:
            ...
            for column in columns:
                ...
                if column_type == 'char':
                    ...
                    col_value = res[col - 1].strftime('%d - %m - %Y') if res[col - 1] else ''
                    wbf_value = wbf['content']
                else:
    ...
    def add_workbook_format(self, workbook):
        ...
        wbf['content_datetime'] = workbook.add_format({'num_format': 'dd - mm - yyyy', 'font_name': 'Georgia'})
        wbf['content_datetime'].set_left()
        wbf['content_datetime'].set_right()

        wbf['content_date'] = workbook.add_format({'num_format': 'dd - mm - yyyy', 'font_name': 'Georgia'})
        ...
        wbf['company'] = workbook.add_format({'align': 'center', 'font_name': 'Georgia'})
        ...
        wbf['content_float'] = workbook.add_format(
            {'align': 'center', 'num_format': '#,##0.00', 'font_name': 'Georgia'})
        wbf['content_float'].set_right()
        wbf['content_float'].set_left()

        wbf['content_number'] = workbook.add_format({'align': 'center', 'num_format': '#,##0', 'font_name': 'Georgia'})
        wbf['content_number'].set_right()
        wbf['content_number'].set_left()

        wbf['content_percent'] = workbook.add_format({'align': 'center', 'num_format': '0.00%', 'font_name': 'Georgia'})
        wbf['content_percent'].set_right()
        wbf['content_percent'].set_left()

        wbf['total_float'] = workbook.add_format(
            {'bold': 1, 'bg_color': colors['white_orange'], 'align': 'center', 'num_format': '#,##0.00',
             'font_name': 'Georgia'})
        ...
        wbf['total_number'] = workbook.add_format(
            {'align': 'center', 'bg_color': colors['white_orange'], 'bold': 1, 'num_format': '#,##0',
             'font_name': 'Georgia'})
        ...
        wbf['total_float_yellow'] = workbook.add_format(
            {'bold': 1, 'bg_color': colors['yellow'], 'align': 'center', 'num_format': '#,##0.00',
             'font_name': 'Georgia'})
        ...
        wbf['total_number_yellow'] = workbook.add_format(
            {'align': 'center', 'bg_color': colors['yellow'], 'bold': 1, 'num_format': '#,##0', 'font_name': 'Georgia'})
        ...
        wbf['total_float_orange'] = workbook.add_format(
            {'bold': 1, 'bg_color': colors['orange'], 'align': 'center', 'num_format': '#,##0.00',
             'font_name': 'Georgia'})
        ...
        wbf['total_number_orange'] = workbook.add_format(
            {'align': 'center', 'bg_color': colors['orange'], 'bold': 1, 'num_format': '#,##0', 'font_name': 'Georgia'})
