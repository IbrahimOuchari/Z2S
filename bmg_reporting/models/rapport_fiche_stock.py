...
class ReportStockCardReportXlsx(models.AbstractModel):
    ...
    def _get_ws_params(self, wb, data, product):
        ...
        ws_params = {
            ...
            "title": "Rapport de Fiche de Stock - {}".format(product.name),
...
class StockCardReport(models.TransientModel):
    ...
    date_to = fields.Date()
    product_ids = fields.Many2many(comodel_name="product.product", domain=[('type', '=', 'product')])
    location_id = fields.Many2one(comodel_name="stock.location")
    ...
    def print_report(self, report_type="qweb"):
        ...
        action = (
            report_type == "xlsx"
            and self.env.ref("bmg_reporting.action_rapport_fiche_stock_xlsx")
            or self.env.ref("bmg_reporting.action_rapport_fiche_stock_pdf")
        )
    ...
    def _get_html(self):
        ...
        if report:
            ...
            result["html"] = self.env.ref(
                "bmg_reporting.report_rapport_fiche_stock_html"
            )._render(rcontext)
        return result
