...
class StockCardReportWizard(models.TransientModel):
    ...
    date_range_id = fields.Many2one(comodel_name="date.range", string="Période")
    date_from = fields.Date(string="Date Début")
    date_to = fields.Date(string="Date Fin")
    location_id = fields.Many2one(
        comodel_name="stock.location", string="Emplacement", required=True, domain=[("usage", "in", ["view", "internal"])]
    )
    product_ids = fields.Many2many(
        comodel_name="product.product", string="Produit", required=True, domain=[('type', '=', 'product')]
    )
    ...
    def button_export_html(self):
        self.ensure_one()
        action = self.env.ref("bmg_reporting.action_report_rapport_fiche_stock_html")
