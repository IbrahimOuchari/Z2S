...
class ReturnComponentsWizard(models.TransientModel):
    ...
    def update_mrp_production_state(self):
        """Update the state of the associated mrp.production based on quantity_left."""
        if self.mrp_production_id:
            all_lines = self.line_ids
            all_zero = all(line.quantity_left == 0 for line in all_lines)

            if all_zero:
                self.mrp_production_id.state = 'all_returned'
                _logger.info(_("Production %s marked as all returned.") % self.mrp_production_id.name)
            else:
                self.mrp_production_id.state = 'in_progress'  # You can set to whatever state you want
                _logger.info(_("Production %s has remaining components.") % self.mrp_production_id.name)
