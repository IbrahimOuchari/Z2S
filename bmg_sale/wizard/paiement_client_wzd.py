...
class AccountVoucherWizard(models.TransientModel):
    ...
    amount_total = fields.Monetary("Amount total", readonly=True, digits='Product Price')
    ...
    currency_amount = fields.Monetary(
        "Curr. amount", readonly=True, currency_field="currency_id", digits='Product Price'
    )
    ...
    def check_amount(self):
        if self.amount_advance <= 0:
            raise exceptions.ValidationError(_("Montant du paiement doit être positif"))
        if self.env.context.get("active_id", False):
            ...
            if (
                ...
                raise exceptions.ValidationError(
                    _("Le montant du paiement est supérieur au montant de la commande")
                )
        ...
        if self.amount_advance < 0.0:
            raise UserError(
                _(
                    ...
                    "Veuillez utiliser le type de paiement pour indiquer s'il s'agit d'un paiement entrant ou sortant"
                )
            )
