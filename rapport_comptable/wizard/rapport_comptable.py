...
D_LEDGER = {'general': {'name': _('Grand Livre'),
                        'group_by': 'account_id',
                        'model': 'account.account',
                        'short': 'code',
                        },
            'partner': {'name': _('Livre des Tiers'),
                        'group_by': 'partner_id',
                        'model': 'res.partner',
                        'short': 'name',
                        },
            'journal': {'name': _('Journal'),
                        'group_by': 'journal_id',
                        'model': 'account.journal',
                        'short': 'code',
                        },
            'open': {'name': _('Journal Ouverture'),
                     'group_by': 'account_id',
                     'model': 'account.account',
                     'short': 'code',
                     },
            'aged': {'name': _('Balance Âgée'),
                     'group_by': 'partner_id',
                     'model': 'res.partner',
                     'short': 'name',
                     },
            'analytic': {'name': _('Grand Livre Analytique'),
...
class AccountStandardLedgerReport(models.TransientModel):
    ...
    date_from = fields.Date(string='Date Début', help='Utiliser pour calculer le solde initial.')
    date_to = fields.Date(string='Date Fin', help='Utiliser pour calculer ensemble correspondant au futur.')
...
class AccountStandardLedgerLines(models.TransientModel):
    ...
    move_id = fields.Many2one('account.move', 'Réf')
    move_line_id = fields.Many2one('account.move.line')
    date = fields.Date()
    date_maturity = fields.Date('Due Date')
    debit = fields.Monetary(default=0.0, currency_field='company_currency_id', digits='Product Price')
    credit = fields.Monetary(default=0.0, currency_field='company_currency_id', digits='Product Price')
    balance = fields.Monetary(default=0.0, currency_field='company_currency_id', digits='Product Price')
    cumul_balance = fields.Monetary(default=0.0, currency_field='company_currency_id', string='Balance Cumulée', digits='Product Price')
    ...
    current = fields.Monetary(default=0.0, currency_field='company_currency_id', digits='Product Price', string='Pas due')
    age_30_days = fields.Monetary(default=0.0, currency_field='company_currency_id', digits='Product Price', string='30 Jours')
    age_60_days = fields.Monetary(default=0.0, currency_field='company_currency_id', digits='Product Price', string='60 Jours')
    age_90_days = fields.Monetary(default=0.0, currency_field='company_currency_id', digits='Product Price', string='90 Jours')
    age_120_days = fields.Monetary(default=0.0, currency_field='company_currency_id', digits='Product Price', string='120 Jours')
    older = fields.Monetary(default=0.0, currency_field='company_currency_id', digits='Product Price', string='Plus Ancien')

    amount_currency = fields.Monetary(default=0.0, currency_field='currency_id', string='Montant Devise', digits='Product Price')
...
class AccountStandardLedger(models.TransientModel):
    ...
    account_in_ex_clude_ids = fields.Many2many(comodel_name='account.account', string='Comptes',
                                               check_company=True,
                                               help='Si vide, obtenir tous les comptes')
    account_group_ids = fields.Many2many(comodel_name='account.group', string='Groupe de Compte')
    ...
    target_move = fields.Selection([('posted', 'Toutes les Entrées Comptablisées'),
    ...
    result_selection = fields.Selection([('customer', 'Clients'),
                                         ...
                                         ('customer_supplier', 'Clients & Fournisseurs')
    ...
    report_type = fields.Selection([('account', 'Account'), ('partner', 'Partner'), ('journal', 'Journal'),
                                    ('analytic', 'Analytic')], string='Type Rapport')
    ...
    def action_view_lines(self):
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'tree,form',
            'views': [(self.env.ref('rapport_comptable.view_aged_tree').id if self.ledger_type == 'aged' else False, 'tree'), (False, 'form')],
    ...
    def print_pdf_report(self):
        self.ensure_one()
        self._compute_data()
        return self.env.ref('rapport_comptable.action_standard_report').report_action(self)

    def print_excel_report(self):
        self.ensure_one()
        self._compute_data()
        print(self.env.ref('rapport_comptable.action_standard_excel').report_action(self))
        return self.env.ref('rapport_comptable.action_standard_excel').report_action(self)
