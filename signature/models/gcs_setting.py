...
class GcsSettings(models.TransientModel):
    ...
    signature_option = fields.Selection(
        selection=[('company_signature', 'Signature de Société'), ('individual_signature', 'Signature par User')],
        string='Option Signature',
        config_parameter='directory.path.signature_option'
    )
    sale_order = fields.Boolean(string='Commande de Vente', config_parameter='directory.path.sale_order_option')
    invoice = fields.Boolean(string='Facture', config_parameter='directory.path.invoice')
    purchase_order = fields.Boolean(string='Achat', config_parameter='directory.path.purchase_order')
    sale_devis = fields.Boolean(string='Devis',
                                config_parameter='directory.path.sale_devis')
    purchase_rfq = fields.Boolean(string='Devis',
                                config_parameter='directory.path.purchase_rfq')
    signature_position = fields.Boolean(string='Position de la signature à gauche',
    ...
    def get_sale_devis(self):
        sale_devis = self._get_settings("directory.path.sale_devis", "Devis")
        return sale_devis

    def get_purchase_rfq(self):
        purchase_rfq = self._get_settings("directory.path.purchase_rfq", "DA")
        return purchase_rfq
    ...
    def get_values(self):
        ...
        sale_devis = self.env["ir.config_parameter"].sudo().get_param("directory.path.sale_devis")
        purchase_rfq = self.env["ir.config_parameter"].sudo().get_param("directory.path.purchase_rfq")
    ...
    def set_values(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param("directory.path.sale_devis",
                                                         self.sale_devis)
        self.env["ir.config_parameter"].sudo().set_param("directory.path.purchase_rfq",
                                                         self.purchase_rfq)
